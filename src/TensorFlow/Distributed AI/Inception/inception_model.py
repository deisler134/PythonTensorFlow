...
def inference(images, num_classes, for_training=False, restore_logits=True,
  ...
  with slim1.arg_scope([slim1.ops.conv2d, slim1.ops.fc], weight_decay=0.00004):
    with slim1.arg_scope([slim1.ops.conv2d],
                        stddev=0.1,
                        activation=tf.nn.relu,
                        batch_norm_params=batch_norm_params):
      logits, endpoints = slim1.inception.inception_v3(
          images,
...
def loss(logits, labels, batch_size=None):
  ...
  slim1.losses.cross_entropy_loss(logits[0],
                                 dense_labels,
                                 label_smoothing=0.1,
                                 weight=1.0)

  # Cross entropy loss for the auxiliary softmax head.
  slim1.losses.cross_entropy_loss(logits[1],
                                 dense_labels,
                                 label_smoothing=0.1,
                                 weight=0.4,
                                 scope='aux_loss')
