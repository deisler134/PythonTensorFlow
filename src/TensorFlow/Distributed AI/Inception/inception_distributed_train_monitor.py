...
def train(target, dataset, cluster_spec):
  ...
  print('!!!!!!!!worker num: %d' % num_replicas_to_aggregate)
  # Ops are assigned to worker by default.
  with tf.device(
    '/job:worker/task:%d' % FLAGS.task_id
#     tf.train.replica_device_setter(
#         worker_device="/job:worker/task:%d/GPU:%d" % FLAGS.task_id,FLAGS.task_id,
...
#       saver = tf.train.Saver()
      ...
      with tf.train.MonitoredTrainingSession(
                            ...
                            hooks=[sync_replicas_hook,summary_hook],
                            config=sess_config
                            ) as sess:
...
#            sv.start_queue_runners(sess, chief_queue_runners)
            tf.train.start_queue_runners(sess,collection=chief_queue_runners)
